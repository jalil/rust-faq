...
impl Store {
    pub async fn new(db_url: &str) -> Self {
        ...
        {
            Ok(pool) => pool,
            Err(e) => panic!("Couldt establish DB connection:{}", e),
        };
        Store {
            connection: db_pool,
        }
    ...
    pub async fn get_questions(
        ...
        match sqlx::query("SELECT * from questions LIMIT $1 OFFSET $!")
            .bind(limit)
            .bind(offset)
    ...
    pub async fn delete_question(&self, id: i32, account_id: AccountId) -> Result<bool, Error> {
        match sqlx::query("DELETE FROM question WHERE id = $1 AND account_id=$2")
            .bind(id)
            .bind(account_id.0)
